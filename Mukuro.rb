...
TOKEN = 'NDI3MjAwNjI5NTA5OTgwMTcx.DcUzEA.Mz5RWETwhYRzTQavcyizVV3BCfY'
PREFIX = 'm.'
...
bot.message(with_text: "#{PREFIX}quote") do |event|
    ...
    load 'quotes.rb'
         event.channel.send_embed do |embed|
               ...
               embed.footer = Discordrb::Webhooks::EmbedFooter.new(text: 'Saved from Reborn Mafia', icon_url: 'https://bts2013.files.wordpress.com/2016/07/mafiasmall.jpg?w=393&h=421') 
         end
end
    ...
    if names == nil
        ...
        if args[0] == nil
            ...
            if $value_id == nil
                ...
                event.respond "Added **#{$value_quote}** to **#{bot.user($value_id).name}**"
                File.open("quotes.rb", "a") {|f| f.write("
$quotes['#{$quotes_n}'] = [#{$value_id}, #{$value_server_id}, '#{$value_quote}']") }
            end
        end
    end
end
